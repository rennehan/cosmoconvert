...
class tipsy2gizmo(object):
    ...
    def __init__(self, snapshot_file, box_size, cosmology, aux_file = None,
        ...
        gizmo_dict = dict(PartType0 = {}, PartType1 = {}, PartType4 = {})
        ...
        header['gizmo'] = {}
        if header['time'] > 1.0:
            header['gizmo']['time'] = 1.0
            header['gizmo']['redshift'] = 0.0
        else:
            header['gizmo']['time'] = header['time']
        ...
        header['gizmo']['flag_metals'] = self.gizmo_num_metals
        header['gizmo']['flag_feedback'] = 1
        header['gizmo']['flag_doubleprecision'] = 0
        header['gizmo']['flag_ic_info'] = 0

        util.io.info('Reading %s' % snapshot_file)
        util.io.info('Box size %s Mpc' % box_size)
        util.io.info('TIPSY+GIZMO Header:')

        print(header)

        mass_factor = self.unit_mass / self.gizmo_mass
        length_factor = self.unit_length / self.gizmo_length
        time_factor = self.unit_time / self.gizmo_time
        density_factor = self.unit_density / self.gizmo_density
        specific_energy_factor = 1.0 / self.gizmo_specific_energy
        velocity_factor = self.unit_velocity / self.gizmo_velocity

        # GIZMO has a weird velocity unit, we need this factor.
        velocity_factor *= np.sqrt(header['time'])
